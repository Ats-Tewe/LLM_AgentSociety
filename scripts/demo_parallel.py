...
from src.flows.parallel_flow import ParallelState, YelpParallelFlow


# ---------------------------------------------------------------------------
# Lightweight NDJSON data loader
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Build ParallelState inputs from raw data
# ---------------------------------------------------------------------------

def _build_state(user_id: str, item_id: str, loader: _DataLoader) -> ParallelState:
    ...
    def fake_completion(*args, **kwargs):
        return ModelResponse(
            ...
            choices=[{
                ...
                "message": {
                    "role": "assistant",
                    "content": '{"stars": 4.0, "review": "[Mocked] Parallel flow review output."}',
                },
            }],
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run YelpParallelFlow (user_profiler + item_analyst in parallel, "
            "then prediction_modeler). "
            "Generate real IDs first with: uv run python scripts/sample_dummy_data.py"
        )
    )
    ...
    os.environ["CREWAI_PROCESS_MODE"] = "parallel"

    data_dir = Path(args.data_dir).expanduser().resolve()
    print(f"Loading data from {data_dir} ...")
    loader = _DataLoader(data_dir)

    print(f"Building state for user={args.user_id!r}, item={args.item_id!r} ...")
    state = _build_state(args.user_id, args.item_id, loader)

    patcher = _start_mock_if_needed(args.mock)
    try:
        flow = YelpParallelFlow(initial_state=state)
        result = flow.kickoff()

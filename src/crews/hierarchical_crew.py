...
class HierarchicalCrew():
    """Hierarchical crew with explicit manager agent for Yelp review prediction.

    review_prediction_manager orchestrates four worker specialists:
      user_profiler → item_analyst → calibrator → prediction_modeler

    Data is pre-fetched in CrewAISimulationAgent and passed as template variables.
    The manager validates each intermediate result before moving to the next stage.
    """
    ...
    def calibrate_user_task(self) -> Task:
        return Task(config=self.tasks_config['calibrate_user_task'])
    ...
    def crew(self) -> Crew:
        manager = Agent(
            config=self.agents_config['review_prediction_manager'],
            llm=_build_llm(),
            verbose=False,
